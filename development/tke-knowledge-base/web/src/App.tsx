...
export default function App() {
  ...
  /** Switch to chat view and send a question (from library "Ask AI" button) */
  function handleAskFromLibrary(question: string) {
    setActiveView("chat");
    // Small delay so the view renders first
    setTimeout(() => sendMessage(question, null), 100);
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-tke-bg">
      ...
      {activeView === "chat" && (
        <>
          ...
          <div className="flex-1 overflow-y-auto">
            {messages.length === 0 ? (
              <WelcomeScreen onBrowseLibrary={() => setActiveView("library")} />
            ) : (
      ...
      {/* Library view */}
      {activeView === "library" && <Library onAskAbout={handleAskFromLibrary} />}
    </div>
  );
}
