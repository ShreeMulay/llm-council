...
The fixed synthetic shadow canary uses one fully verified warmup per exact URL and API surface followed by five measured pairs in deterministic `AB, BA, AB, BA, AB` order. Warmups are cold evidence and are excluded from steady aggregates. Any fixed deployment instruction/objective mismatch, factual error, evaluator-format failure, schema violation, explicit error, fallback, route mismatch, policy violation, usage/pricing violation, or absolute latency/token/cost SLO violation by the candidate hard-fails the canary regardless of the immutable benchmark outcome. Paired steady-state latency and list-cost ratios are explicit diagnostic statuses only; they do not duplicate or override the representative benchmark's relative promotion decision. These artifacts make no claim of comprehensive safety evaluation.

## Deployment

Build from the reviewed merged tree and pin the image digest plus registry/projection digests. Prospectively, the next rollout MUST verify the immutable benchmark before mutation, deploy a zero-traffic tagged revision, run the paired synthetic canary after strict shadow health, then use strict candidate absolute checks through staged 10% and 50% canaries before 100%. Rehearse atomic rollback to the prior image and registry before final promotion. This contract does not claim that deployment has completed. Verify Forgejo/GitHub/master/shared-checkout equality at closeout.
