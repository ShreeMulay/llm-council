...
def format_council_markdown(
    ...
    if include_details and stage1_results:
        ...
        for result in stage1_results:
            ...
            lines.append(f"<summary><strong>{model}</strong> ({provider}, {tokens} tokens)</summary>\n")
    ...
    if stage2_results and include_details:
        ...
        if metadata.get('aggregate_rankings'):
            ...
            for i, ranking in enumerate(metadata['aggregate_rankings']):
                ...
                lines.append(f"| {rank_display} | {ranking['model']} | {ranking['average_rank']} | {ranking['rankings_count']} |")
            lines.append("")
        ...
        for ranking in stage2_results:
            ...
            lines.append(f"\n**{model}'s evaluation**:")
            lines.append(f"Ranking: {' > '.join(parsed) if parsed else 'Could not parse'}")
            lines.append(f"\n{evaluation}\n")
        lines.append("</details>\n")
    ...
    lines.append(f"*Council completed in {elapsed_seconds:.1f}s using {len(stage1_results)} models | ~{total_tokens:,} tokens*")
